<script setup lang="ts">
interface Props {
  data: Record<string, unknown>[]
  loading?: boolean
  skeletonCount?: number
  rowKey?: string
  selectable?: boolean
  hoverable?: boolean
  divided?: boolean
  compact?: boolean
  emptyTitle?: string
  emptyDescription?: string
  emptyIcon?: 'inbox' | 'search' | 'document' | 'users' | 'folder' | 'chart'
}

const props = withDefaults(defineProps<Props>(), {
  loading: false,
  skeletonCount: 5,
  rowKey: 'id',
  selectable: false,
  hoverable: true,
  divided: true,
  compact: false,
  emptyTitle: '',
  emptyDescription: '',
  emptyIcon: 'inbox',
})

const emit = defineEmits<{
  'item-click': [item: Record<string, unknown>, index: number]
  select: [selectedItems: Record<string, unknown>[]]
}>()

const selectedKeys = ref<Set<string | number>>(new Set())

const selectedItems = computed(() => {
  return props.data.filter((item) => selectedKeys.value.has(getItemKey(item, 0)))
})

const allSelected = computed(() => {
  if (props.data.length === 0) return false
  return props.data.every((item, index) => selectedKeys.value.has(getItemKey(item, index)))
})

const someSelected = computed(() => {
  if (props.data.length === 0) return false
  const count = props.data.filter((item, index) => selectedKeys.value.has(getItemKey(item, index))).length
  return count > 0 && count < props.data.length
})

const emptyIconPaths: Record<string, string> = {
  inbox: 'M2.25 13.5h3.86a2.25 2.25 0 012.012 1.244l.256.512a2.25 2.25 0 002.013 1.244h3.218a2.25 2.25 0 002.013-1.244l.256-.512a2.25 2.25 0 012.013-1.244h3.859m-19.5.338V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18v-4.162c0-.224-.034-.447-.1-.661L19.24 5.338a2.25 2.25 0 00-2.15-1.588H6.911a2.25 2.25 0 00-2.15 1.588L2.35 13.177a2.25 2.25 0 00-.1.661z',
  search: 'M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z',
  document: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z',
  users: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z',
  folder: 'M2.25 12.75V12A2.25 2.25 0 014.5 9.75h15A2.25 2.25 0 0121.75 12v.75m-8.69-6.44l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z',
  chart: 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z',
}

function getItemKey(item: Record<string, unknown>, index: number): string | number {
  return (item[props.rowKey] as string | number) ?? index
}

function handleItemClick(item: Record<string, unknown>, index: number) {
  emit('item-click', item, index)
}

function toggleSelectAll() {
  if (allSelected.value) {
    selectedKeys.value.clear()
  } else {
    props.data.forEach((item, index) => {
      selectedKeys.value.add(getItemKey(item, index))
    })
  }
  emit('select', selectedItems.value)
}

function toggleItemSelection(item: Record<string, unknown>, index: number) {
  const key = getItemKey(item, index)
  if (selectedKeys.value.has(key)) {
    selectedKeys.value.delete(key)
  } else {
    selectedKeys.value.add(key)
  }
  emit('select', selectedItems.value)
}

function isItemSelected(item: Record<string, unknown>, index: number): boolean {
  return selectedKeys.value.has(getItemKey(item, index))
}

function clearSelection() {
  selectedKeys.value.clear()
  emit('select', [])
}

const { t } = useLocale()

defineExpose({
  selectedItems,
  clearSelection,
})
</script>

<template>
  <div class="data-list">
    <!-- Loading State -->
    <div
      v-if="loading"
      class="space-y-1"
    >
      <div
        v-for="i in skeletonCount"
        :key="`skeleton-${i}`"
        :class="[
          'flex items-center gap-4 animate-pulse',
          compact ? 'py-2 px-3' : 'py-4 px-4',
        ]"
      >
        <div
          v-if="selectable"
          class="skeleton w-4 h-4 rounded"
        />
        <div class="skeleton w-10 h-10 rounded-full flex-shrink-0" />
        <div class="flex-1">
          <div class="skeleton h-4 w-1/3 rounded mb-2" />
          <div class="skeleton h-3 w-1/2 rounded" />
        </div>
        <div class="skeleton h-4 w-16 rounded" />
      </div>
    </div>

    <!-- Empty State -->
    <div
      v-else-if="data.length === 0"
      class="flex flex-col items-center justify-center py-12 px-4"
    >
      <div class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mb-4">
        <svg
          class="w-8 h-8 text-gray-400"
          fill="none"
          viewBox="0 0 24 24"
          stroke-width="1.5"
          stroke="currentColor"
        >
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            :d="emptyIconPaths[emptyIcon]"
          />
        </svg>
      </div>
      <h3 class="text-lg font-medium text-gray-900 mb-1">
        {{ emptyTitle || t('table.noData') }}
      </h3>
      <p class="text-sm text-gray-900 text-center max-w-sm">
        {{ emptyDescription || t('table.noDataDefault') }}
      </p>
      <slot name="empty-action" />
    </div>

    <!-- Data List -->
    <div
      v-else
      class="bg-white rounded border border-gray-200 overflow-hidden"
    >
      <!-- Select All Header -->
      <div
        v-if="selectable"
        :class="[
          'flex items-center gap-3 bg-gray-50 border-b border-gray-200',
          compact ? 'py-2 px-3' : 'py-3 px-4',
        ]"
      >
        <input
          type="checkbox"
          :checked="allSelected"
          :indeterminate="someSelected"
          class="form-check-input"
          @change="toggleSelectAll"
        >
        <span class="text-sm text-gray-900">
          {{ selectedKeys.size > 0 ? `${selectedKeys.size} ${t('table.selected')}` : t('table.selectAll') }}
        </span>
      </div>

      <!-- List Items -->
      <ul class="divide-y divide-gray-100">
        <li
          v-for="(item, index) in data"
          :key="getItemKey(item, index)"
          :class="[
            'flex items-center gap-3 transition-colors',
            compact ? 'py-2 px-3' : 'py-4 px-4',
            { 'hover:bg-gray-50 cursor-pointer': hoverable },
            { 'bg-primary-50': isItemSelected(item, index) },
          ]"
          @click="handleItemClick(item, index)"
        >
          <!-- Checkbox -->
          <div
            v-if="selectable"
            class="flex-shrink-0"
            @click.stop
          >
            <input
              type="checkbox"
              :checked="isItemSelected(item, index)"
              class="form-check-input"
              @change="toggleItemSelection(item, index)"
            >
          </div>

          <!-- Item Content -->
          <div class="flex-1 min-w-0">
            <slot
              :item="item"
              :index="index"
              :selected="isItemSelected(item, index)"
            >
              <!-- Default Rendering -->
              <div class="flex items-center gap-3">
                <slot
                  name="avatar"
                  :item="item"
                  :index="index"
                >
                  <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                    <span class="text-sm font-medium text-gray-900">
                      {{ String(item[rowKey] || index + 1).charAt(0).toUpperCase() }}
                    </span>
                  </div>
                </slot>
                <div class="flex-1 min-w-0">
                  <slot
                    name="title"
                    :item="item"
                    :index="index"
                  >
                    <p class="text-sm font-medium text-gray-900 truncate">
                      Item {{ index + 1 }}
                    </p>
                  </slot>
                  <slot
                    name="subtitle"
                    :item="item"
                    :index="index"
                  >
                    <p class="text-xs text-gray-900 truncate">
                      {{ Object.keys(item).length }} {{ t('table.properties') }}
                    </p>
                  </slot>
                </div>
              </div>
            </slot>
          </div>

          <!-- Actions -->
          <div class="flex-shrink-0">
            <slot
              name="actions"
              :item="item"
              :index="index"
            />
          </div>
        </li>
      </ul>
    </div>

    <!-- Selection Info -->
    <div
      v-if="selectable && selectedKeys.size > 0"
      class="mt-3 px-4 py-2 bg-primary-50 rounded flex items-center justify-between"
    >
      <span class="text-sm text-primary-700">
        {{ selectedKeys.size }} {{ t('table.elementsSelected') }}
      </span>
      <button
        type="button"
        class="text-sm text-primary-600 hover:text-primary-800 font-medium"
        @click="clearSelection"
      >
        {{ t('table.clearSelection') }}
      </button>
    </div>
  </div>
</template>
